...
void spmm2_fp32_scalar(config_t *config,
                       input_t *input,
                       output_t *output) {
    ...
    for (int layer = 0; layer < layer_count; layer++) {
        ...
        for (int m = 0; m < M; m += MR) {
            ...
            for (int n = 0; n < N; n++) {
                float acc[MR];
                float bias_data = bias_vector[n];
                for (int mr = 0; mr < r_MR; mr++)
                    acc[mr] = bias_data;
                for (int k = weight_NNZ_vector[n]; k < weight_NNZ_vector[n + 1]; k++) {
                    float weight_data = weight_value_vector[k];
                    float *intput_addr = input_matrix + weight_IDX_vector[k] * M;
                    for (int mr = 0; mr < r_MR; mr++) {
                        acc[mr] += (weight_data * (*intput_addr));
                        intput_addr++;
                    }
                }
                float *output_addr = output_matrix + n * M;
                for (int mr = 0; mr < r_MR; mr++) {
...
void spmm2_int32_scalar(config_t *config,
                        input_t *input,
                        output_t *output) {
    ...
    for (int layer = 0; layer < layer_count; layer++) {
        ...
        for (int m = 0; m < M; m += MR) {
            ...
            for (int n = 0; n < N; n++) {
                int32_t acc[MR];
                int32_t bias_data = bias_vector[n];
                for (int mr = 0; mr < r_MR; mr++)
                    acc[mr] = bias_data;
                for (int k = weight_NNZ_vector[n]; k < weight_NNZ_vector[n + 1]; k++) {
                    int32_t weight_data = weight_value_vector[k];
                    int32_t *intput_addr = input_matrix + weight_IDX_vector[k] * M;
                    for (int mr = 0; mr < r_MR; mr++) {
                        acc[mr] += (weight_data * (*intput_addr));
                        intput_addr++;
                    }
                }
                int32_t *output_addr = output_matrix + n * M;
                for (int mr = 0; mr < r_MR; mr++) {
                    acc[mr] = acc[mr] > max ? max : acc[mr];
                    acc[mr] = acc[mr] < min ? min : acc[mr];
                    *output_addr = acc[mr];
...
void spmm2_fp16_scalar(config_t *config,
                       input_t *input,
                       output_t *output) {
    ...
    for (int layer = 0; layer < layer_count; layer++) {
        ...
        for (int m = 0; m < M; m += MR) {
            ...
            for (int n = 0; n < N; n++) {
                __fp16 acc[MR];
                __fp16 bias_data = bias_vector[n];
                for (int mr = 0; mr < r_MR; mr++)
                    acc[mr] = bias_data;
                for (int k = weight_NNZ_vector[n]; k < weight_NNZ_vector[n + 1]; k++) {
                    __fp16 weight_data = weight_value_vector[k];
                    __fp16 *intput_addr = input_matrix + weight_IDX_vector[k] * M;
                    for (int mr = 0; mr < r_MR; mr++) {
                        acc[mr] += (weight_data * (*intput_addr));
                        intput_addr++;
                    }
                }
                __fp16 *output_addr = output_matrix + n * M;
                for (int mr = 0; mr < r_MR; mr++) {
                    acc[mr] = acc[mr] > max ? max : acc[mr];
                    acc[mr] = acc[mr] < min ? min : acc[mr];
                    *output_addr = acc[mr];
...
void spmm2_int16_scalar(config_t *config,
                        input_t *input,
                        output_t *output) {
    ...
    for (int layer = 0; layer < layer_count; layer++) {
        ...
        for (int m = 0; m < M; m += MR) {
            ...
            for (int n = 0; n < N; n++) {
                int16_t acc[MR];
                int16_t bias_data = bias_vector[n];
                for (int mr = 0; mr < r_MR; mr++)
                    acc[mr] = bias_data;
                for (int k = weight_NNZ_vector[n]; k < weight_NNZ_vector[n + 1]; k++) {
                    int16_t weight_data = weight_value_vector[k];
                    int16_t *intput_addr = input_matrix + weight_IDX_vector[k] * M;
                    for (int mr = 0; mr < r_MR; mr++) {
                        acc[mr] += (weight_data * (*intput_addr));
                        intput_addr++;
                    }
                }
                int16_t *output_addr = output_matrix + n * M;
                for (int mr = 0; mr < r_MR; mr++) {
                    acc[mr] = acc[mr] > max ? max : acc[mr];
                    acc[mr] = acc[mr] < min ? min : acc[mr];
                    *output_addr = acc[mr];
