...
void gemm2_fp32_scalar(config_t *config,
                       input_t *input,
                       output_t *output) {
...
void gemm2_int32_scalar(config_t *config,
                        input_t *input,
                        output_t *output) {
...
void gemm2_fp16_scalar(config_t *config,
                       input_t *input,
                       output_t *output) {
    ...
    for (int layer = 0; layer < layer_count; layer++) {
        ...
        for (int m = 0; m < M; m += MR) {
            int r_MR = M - m > MR ? MR : M - m;
            __fp16 *weight_addr = weight_matrix;
            for (int n = 0; n < N; n++) {
                __fp16 acc[MR];
                __fp16 bias_data = bias_vector[n];
                for (int mr = 0; mr < r_MR; mr++)
                    acc[mr] = bias_data;
                __fp16 *intput_addr = input_matrix;
                for (int k = 0; k < K; k++) {
                    __fp16 *input_addr_temp = intput_addr;
                    for (int mr = 0; mr < r_MR; mr++) {
                        acc[mr] += (*(weight_addr) * (*input_addr_temp));
                        input_addr_temp++;
                    }
...
void gemm2_int16_scalar(config_t *config,
                        input_t *input,
                        output_t *output) {
